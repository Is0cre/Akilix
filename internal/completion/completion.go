...
_pensuse() {
  if (( CURRENT >= 4 )) && [[ "$words[1]" == "pensuse" && "$words[2]" == "workbook" ]]; then
    case "$words[3]" in
      open|overview|follow|path|status|close|reopen|rename|validate) _pensuse_workbooks; return ;;
    esac
  fi
  if (( CURRENT >= 5 )) && [[ "$words[1]" == "pensuse" && "$words[2]" == "evidence" && "$words[3]" == "verify" ]]; then _pensuse_evidence_ids; return; fi
  if (( CURRENT >= 6 )) && [[ "$words[1]" == "pensuse" && "$words[2]" == "container" && "$words[3]" == "run" ]]; then _values 'options' --mount-originals --mount-output --target --override --json --workdir --env --; return; fi
  ...
  case $state in
    ...
    argument)
      case $words[2] in
        workbook) _values 'workbook command' create list open overview follow path status close reopen rename validate; [[ $CURRENT -ge 4 ]] && _pensuse_workbooks ;;
...
const Bash = `_pensuse_complete() {
  ...
  if [[ ${COMP_CWORD} -eq 1 ]]; then COMPREPLY=($(compgen -W 'version workbook scope logging evidence run container tui profile repository config bar completion' -- "$cur")); return; fi
