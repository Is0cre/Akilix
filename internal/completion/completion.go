  ...
  case $state in
    ...
    argument)
      case $words[2] in
		...
		acquire) _values 'acquisition command' inspect record identify protect ;;
...
const Bash = `_akilix_complete() {
  ...
  if [[ ${COMP_WORDS[1]} == acquire ]]; then COMPREPLY=($(compgen -W 'inspect record identify protect --json' -- "$cur")); return; fi
