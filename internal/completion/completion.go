...
_pensuse() {
  ...
  if (( CURRENT >= 6 )) && [[ "$words[1]" == "pensuse" && "$words[2]" == "container" && "$words[3]" == "run" ]]; then _values 'options' --mount-originals --target --override --json --workdir --env --; return; fi
...
const Bash = `_pensuse_complete() {
  ...
  if [[ ${COMP_WORDS[1]} == container && ${COMP_WORDS[2]} == run && ${COMP_CWORD} -ge 5 ]]; then COMPREPLY=($(compgen -W '--mount-originals --target --override --json --workdir --env --' -- "$cur")); return; fi
