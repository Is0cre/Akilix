...
_akilix() {
  if (( CURRENT >= 4 )) && [[ "$words[1]" == "akilix" && "$words[2]" == "workbook" ]]; then
    case "$words[3]" in
      open|activate|overview|discoveries|follow|path|status|close|reopen|rename|validate) _akilix_workbooks; return ;;
    esac
  fi
  ...
  case $state in
    ...
    argument)
      case $words[2] in
        workbook) _values 'workbook command' create list open activate active deactivate overview discoveries follow path status close reopen rename validate; [[ $CURRENT -ge 4 ]] && _akilix_workbooks ;;
...
const Bash = `_akilix_complete() {
  ...
  if [[ ${COMP_WORDS[1]} == workbook && ${COMP_CWORD} -eq 2 ]]; then COMPREPLY=($(compgen -W 'create list open activate active deactivate overview discoveries follow path status close reopen rename validate' -- "$cur")); return; fi
