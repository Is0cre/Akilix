# Akilix branding assets

This package faithfully derives Linux and software branding assets from the supplied Akilix concept artwork. The detailed monitor-lizard identity is preserved as raster master artwork rather than replaced by a low-quality geometric vector approximation.

## Canonical artwork

- `source/akilix-master.png` — full natural-history monitor lizard master.
- `source/akilix-mark-master.png` — compact raised monitor-head/scope master.
- `source/akilix-wordmark.svg` — editable Akilix wordmark typography.

The concept boards in `reference/` are art direction and are not distributable logos. The monitor lizard is an independent Akilix project identity and is not the openSUSE Geeko logo. These assets do not imply endorsement by SUSE or openSUSE.
