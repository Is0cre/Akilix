...
def main():
    ...
    save(darklogo,'web/pensuse-horizontal.png',(1200,420),BG); save(darklogo,'web/pensuse-horizontal@2x.png',(2400,840),BG)
    save(darklogo,'web/pensuse-horizontal-dark.png',(1200,420),BG); save(lightlogo,'web/pensuse-horizontal-light.png',(1200,420),'#E8E6DD')
    save(HEAD,'web/pensuse-mark.png',(900,900)); save(HEAD,'desktop/pensuse.svg') if False else None
    ...
    for s in [(1920,1080),(2560,1440),(3840,2160)]:
        wall=Image.new('RGBA',s,BG); place(wall,FULL,(int(s[0]*.53),int(s[1]*.35),int(s[0]*.44),int(s[1]*.55))); save(wall,f'os/wallpaper/pensuse-{s[0]}x{s[1]}.png')
    sticker=Image.new('RGBA',(1200,1200),'#E8E6DD'); place(sticker,FULL,(40,40,720,820)); d=ImageDraw.Draw(sticker); d.text((750,480),'PenSUSE',font=font(72,True),fill=GRAPHITE); d.text((755,570),'Security work with provenance.',font=font(20),fill=SLATE); save(sticker,'print/pensuse-sticker.png')
    contact=Image.new('RGB',(1600,1380),'#20282B'); cd=ImageDraw.Draw(contact); cd.text((40,25),'PenSUSE BRANDING REVIEW',font=font(32,True),fill=BONE)
    def terminal_card(lines):
        ...
        return card
    cards=[('PRIMARY',darklogo),('COMPACT',HEAD),('16px × 12',HEAD.resize((192,192),Image.Resampling.NEAREST)),('32px × 12',HEAD.resize((384,384),Image.Resampling.NEAREST)),('SOCIAL',Image.open(ROOT/'web/social-card-1200x630.png')),('PLYMOUTH',ply),('GRUB',grub),('WALLPAPER',Image.open(ROOT/'os/wallpaper/pensuse-1920x1080.png')),('TERMINAL FULL',terminal_card(['                         __','                    _.-\'  `-._','              _..--\'  _      `-._','       ______/      _/ \\_         `\\','PenSUSE  Security work with provenance.'])),('TERMINAL SMALL',terminal_card(['      __',' _..-\'  `-._','/___  ___  _\\','   `-o-o-o-\'  PenSUSE']))]
